...
def get_relevant_codes(selector, queries: List[str], icd_codes_df: pd.DataFrame, top_k: int = 10) -> Dict[str, List[Tuple[str, str, float]]]:
    """
    ...
    # Get ICD code description embeddings
...
def map_query_to_icd(selector, specialty_queries: Dict[str, List[str]], icd_codes_df: pd.DataFrame, top_k: int = 10) -> Dict[str, Dict[str, List[Tuple[str, str, float]]]]:
    ...
    for specialty, queries in specialty_queries.items():
        ...
        # Get relevant codes for all queries in this specialty
        query_to_codes = get_relevant_codes(selector, queries, icd_codes_df, top_k)
