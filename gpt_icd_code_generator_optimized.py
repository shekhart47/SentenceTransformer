Here's a reimplemented solution with a persistent queue approach, which handles rate limiting gracefully and allows for easy resumption after interruptions. I've included detailed usage examples in the comments:
...
def process_with_queue(chunk_index=0, total_chunks=1, calls_per_minute=30, min_delay=1.0):
    """
    ...
    input_file = f"{output_dir}splits/gpt_specialties_split_{chunk_index}.json"
    ...
    icd_reference_lookup, multilabel_specialty_data = load_datasets(input_file)
    ...
    if os.path.exists(queue_file):
        ...
        # Calculate which specialties belong to this chunk
        ...
        logger.info(f"Chunk {chunk_index} will process specialties from index {start_idx} to {end_idx-1}")
        
        # Add all queries for assigned specialties to the queue
        for j in range(start_idx, end_idx):
            specialty = all_specialties[j]
            ...
            queries = multilabel_specialty_data.get(specialty, [])
            if not queries:
...
def main():
    ...
    parser.add_argument('--mode', choices=['split', 'process', 'combine'], default='process',
                        help='Operation mode: split input, process chunk, or combine results')
    parser.add_argument('--input', type=str,
                        help='Input file path (for split mode)')
    parser.add_argument('--output', type=str, default="../gpt_triplet_data_v5/gpt_based_positives_set_3/",
    ...
    if args.mode == 'split':
        if not args.input:
            logger.error("Input file is required for split mode")
            return 1
...
if __name__ == "__main__":
    ...
    1. Split input data into chunks:
       python gpt_icd_code_generator.py --mode split --input specialties_data.json --output ../output_dir/ --chunks 4
    
    2. Process a specific chunk with rate limiting:
       python gpt_icd_code_generator.py --mode process --chunk 0 --chunks 4 --rate 15 --delay 2.0
    
    3. Combine results from all chunks:
       python gpt_icd_code_generator.py --mode combine --output ../output_dir/
    
    4. Run all chunks sequentially (for overnight processing):
       for i in {0..3}; do python gpt_icd_code_generator.py --mode process --chunk $i --chunks 4 --rate 20 --delay 3.0; done
...
1. **Persistent Queue System**:
   - Each query is tracked with its status (pending, processing, completed, failed)
   - Progress is saved after every few queries
   - Can be stopped and restarted at any point

2. **Three-Mode Operation**:
   - `split`: Divides input data into chunks (run once)
   - `process`: Processes one chunk (run for each chunk)
   - `combine`: Combines results from all chunks (run after all chunks complete)

3. **Robust Rate Limiting**:
   - Token bucket algorithm prevents exceeding API limits
   - Configurable calls-per-minute and minimum delay
...
5. **Progress Tracking**:
   ...
   - Detailed logging
   - Specialty-level completion tracking

## Usage Examples

### Step 1: Split the input data
```bash
python gpt_icd_code_generator.py --mode split --input specialties_data.json --chunks 4
```

### Step 2: Process each chunk (run these in separate terminals)
```bash
# Terminal 1
python gpt_icd_code_generator.py --mode process --chunk 0 --chunks 4 --rate 15 --delay 2.0

# Terminal 2
python gpt_icd_code_generator.py --mode process --chunk 1 --chunks 4 --rate 15 --delay 2.0

# Terminal 3
python gpt_icd_code_generator.py --mode process --chunk 2 --chunks 4 --rate 15 --delay 2.0

# Terminal 4
python gpt_icd_code_generator.py --mode process --chunk 3 --chunks 4 --rate 15 --delay 2.0
```

### Step 3: Combine the results after all chunks are processed
```bash
python gpt_icd_code_generator.py --mode combine
```

### For severe rate limiting, use even stricter settings:
```bash
python gpt_icd_code_generator.py --mode process --chunk 0 --chunks 4 --rate 5 --delay 5.0
```

### For overnight processing (one chunk at a time):
```bash
for i in {0..3}; do 
  python gpt_icd_code_generator.py --mode process --chunk $i --chunks 4 --rate 10 --delay 3.0
done
```

The script is designed to handle interruptions gracefully. If you stop the script at any point, it will save its progress and you can restart it later. It will automatically pick up where it left off.​​​​​​​​​​​​​​​​
